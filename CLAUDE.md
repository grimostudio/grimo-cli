...
- **Event-driven**: Inter-module communication用 Spring `@EventListener`（**不是** `@ApplicationModuleListener`，見下方說明）。`IncomingMessageEvent` / `OutgoingMessageEvent` 為統一訊息 port；`MessageRouter` 是中央路由，TUI 和 Channel adapters 共用同一管線。
...
#### TuiAdapter 架構（原 GrimoTuiRunner，已完成拆解）

`TuiAdapter`（~374 lines）是精簡後的 TUI 入口，搭配 4 個提取元件：
...
- `MessageRouter`（root，@Component）— 統一訊息管線（IncomingMessageEvent → command/chat → OutgoingMessageEvent）

#### 統一管線規則（Adapter 不可繞過 MessageRouter）

TUI adapter（TuiKeyHandler、TuiAdapter）**不可直接呼叫 CommandExecutor 或 ChatDispatcher**。所有使用者輸入必須走統一管線：
...
inputCallback.onTextSubmit(text)
  → TuiAdapter.processInput()
    → IncomingMessageEvent
      → MessageRouter
        → CommandExecutor / ChatDispatcher
          → OutgoingMessageEvent
            → TuiAdapter.on(OutgoingMessageEvent)
```
